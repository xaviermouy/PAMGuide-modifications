...
Meta <- function(...,atype='PSD',plottype='Both',envi='Air',calib=0,stype = 'MF',Si=-159,Mh=-36,G=0,vADC=1.414,r=50,N=Fs,winname='Hann',lcut=Fs/N,hcut=Fs/2,timestring="",outwrite=1,welch="",chunksize="",linlog = "Log"){
...
fullfile <- file.choose()		#select a file in the target folder
...
for (i in 1:nf){				#loop through files and analyse
	...
	if (i == 1){disppar <- 1} else {disppar <- 0}
	...
	if (i == 1 && conkcomp == 1){conc <- A}
		...
		if (timestring == ""){	#if not time-stamped, ensure time vector is cumulative
